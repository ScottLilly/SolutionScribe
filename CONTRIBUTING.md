...
- **Adding a template**: put it in `SolutionScribe.Core/Templates/`, add a method and a `TemplatePaths` row to `TemplateFileRepository`, and add a command deriving from `CreateSolutionFileCommandBase`. Without the `TemplatePaths` row nobody can override the template with their own copy and the export command will not write it out. Write fill-ins as `<github user>`, `<repository>`, `<nuget package>` or `<security email>`, which the extension substitutes, and leave anything addressed to the person editing the file afterwards in `[square brackets]`, which it does not. `docs/ARCHITECTURE.md` covers the `<!--#if nuget-->` blocks.
